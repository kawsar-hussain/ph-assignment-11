    ...
    <div className="lg:px-20 px-5 lg:min-h-[92vh] h-auto py-10">
      <div className="relative group max-w-2xl mx-auto overflow-hidden rounded-[2.5rem] p-0.5">
        ...
        <div className="relative bg-black/20 backdrop-blur-2xl shadow-2xl text-white border border-white/20 rounded-[2.5rem] z-10 overflow-hidden">
          ...
          <div className="card-body p-8 md:p-12 gap-6 relative z-10">
            <div className="flex flex-col items-center gap-4 mb-4">
              <h2 className="text-3xl font-black text-center italic flex flex-wrap items-center justify-center gap-3">
                LOOKING FOR
            ...
            <div className="text-center space-y-1">
              <h2 className="text-4xl font-black tracking-tight group-hover:text-yellow-300 transition-colors uppercase italic leading-none">{request.recipientName}</h2>
            ...
            {request.message && (
              <div className=" bg-white/10 rounded-2xl p-4 text-sm italic border-l-4 border-yellow-400 backdrop-blur-md">
                <p className="px-4 text-gray-100">{request.message}</p>
              </div>
            )}

            <div className="bg-black/30 rounded-[1.8rem] p-6 border border-white/10 shadow-inner">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div className="flex flex-col gap-1">
                  <span className="text-[10px] font-bold uppercase tracking-widest text-white/40">Hospital</span>
                  <span className="font-bold text-lg leading-tight">{request.hospitalName}</span>
                </div>

                <div className="flex flex-col gap-1">
                  <span className="text-[10px] font-bold uppercase tracking-widest text-white/40">Exact Address</span>
                  <span className="font-bold text-lg leading-tight">{request.fullAddress}</span>
                </div>

                <div className="flex flex-col gap-1 border-t border-white/10 pt-4">
                  <span className="text-[10px] font-bold uppercase tracking-widest text-white/40">Donation Date</span>
            ...
            <div className=" border-t border-white/10 pt-6">
              <div>
                <p className="text-[10px] font-bold uppercase tracking-widest text-white/40 mb-1">Posted By</p>
                <p className="text-xl font-black italic">{request.requesterName}</p>
                <p className="text-xs text-white/60 hover:text-yellow-300 transition-colors cursor-pointer underline decoration-dotted">{request.requesterEmail}</p>
              </div>
            </div>
